...
func (server *Server) loginUser(ctx *gin.Context) {
	...
	accessToken, _, err := server.tokenMaker.CreateToken(user.Username, server.config.TokenSymmetricKey, server.config.AccessTokenDuration, token.TokenTypeAccessToken)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, errorResponse(err))
		return
	}
